...
class RuleTree() {
    private val rootNode = tilbakedatering()
    ...
    fun runRuleChain(sykmelding: Sykmelding, ruleMetadataSykmelding: RuleMetadataSykmelding): Status {
        return rootNode.evaluate(sykmelding, ruleMetadataSykmelding)
    }

    private fun tilbakedatering(): RuleNode = Tilbakedatering(
        yes = Ettersending(
            yes = ok(),
            no = TilbakedateringIntill8Dager(
                yes = tilbakedateringIntill8Dager(),
                no = TilbakedateringIntill30Dager(
