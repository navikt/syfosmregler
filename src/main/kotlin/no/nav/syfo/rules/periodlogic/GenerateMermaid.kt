...
fun main() {
    ...
    periodLogicRuleTree.traverseTree(builder, "root", "root")
...
private fun TreeNode<PeriodLogicRules, PeriodLogicResult>.traverseTree(
    ...
    when (this) {
        ...
        is RuleNode -> {
            ...
            if (yes is ResultNode) {
                val childResult = (yes as ResultNode<PeriodLogicRules, PeriodLogicResult>).result.status
                val childKey = "${currentNodeKey}_$childResult"
                builder.append("    $thisNodeKey($rule) -->|Yes| $childKey($childResult)${getStyle(childResult)}\n")
            } else {
                val childRule = (yes as RuleNode<PeriodLogicRules, PeriodLogicResult>).rule
            ...
            if (no is ResultNode) {
                val childResult = (no as ResultNode<PeriodLogicRules, PeriodLogicResult>).result.status
                val childKey = "${currentNodeKey}_$childResult"
                builder.append("    $thisNodeKey($rule) -->|No| $childKey($childResult)${getStyle(childResult)}\n")
            } else {
                val childRule = (no as RuleNode<PeriodLogicRules, PeriodLogicResult>).rule
                val childKey = "${currentNodeKey}_$childRule"
