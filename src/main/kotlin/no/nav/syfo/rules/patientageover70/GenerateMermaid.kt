...
fun main() {
    ...
    patientAgeOver70RuleTree.traverseTree(builder, "root", "root")
...
private fun TreeNode<PatientAgeOver70Rules, RuleResult>.traverseTree(
    ...
    when (this) {
        ...
        is RuleNode -> {
            ...
            if (yes is ResultNode) {
                val childResult = (yes as ResultNode<PatientAgeOver70Rules, RuleResult>).result.status
                val childKey = "${currentNodeKey}_$childResult"
                builder.append("    $thisNodeKey($rule) -->|Yes| $childKey($childResult)${getStyle(childResult)}\n")
            } else {
                val childRule = (yes as RuleNode<PatientAgeOver70Rules, RuleResult>).rule
            ...
            if (no is ResultNode) {
                val childResult = (no as ResultNode<PatientAgeOver70Rules, RuleResult>).result.status
                val childKey = "${currentNodeKey}_$childResult"
                builder.append("    $thisNodeKey($rule) -->|No| $childKey($childResult)${getStyle(childResult)}\n")
            } else {
                val childRule = (no as RuleNode<PatientAgeOver70Rules, RuleResult>).rule
                val childKey = "${currentNodeKey}_$childRule"
