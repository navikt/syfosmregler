...
enum class HPRRuleChain(override val ruleId: Int?, override val status: Status, override val predicate: (RuleData<RuleMetadata>) -> Boolean) : Rule<RuleData<RuleMetadata>> {
    ...
    BEHANDLER_OPP_IN_HPR(1403, Status.MANUAL_PROCESSING, { (_, _, _, _, doctor) ->
        !doctor.godkjenninger.godkjenning.any {
    ...
    BEHANDLER_SUSP_IN_HPR(1404, Status.MANUAL_PROCESSING, { (_, _, _, _, doctor) ->
        !doctor.godkjenninger.godkjenning.any {
            it.autorisasjon.isAktiv && it.autorisasjon.oid == 7704 && it.autorisasjon.verdi == "1"
        }
    }),
    ...
    BEHANDLER_ANDRE_IN_HPR(1405, Status.MANUAL_PROCESSING, { (_, _, _, _, doctor) ->
        !doctor.godkjenninger.godkjenning.any {
            it.autorisasjon.isAktiv && it.autorisasjon.oid == 7704 && it.autorisasjon.verdi == "1"
        }
    }),
    ...
    BEHANDLER_NOT_LE_KI_MT_TL_IN_HPR(1407, Status.MANUAL_PROCESSING, { (_, _, _, _, doctor) ->
        !doctor.godkjenninger.godkjenning.any {
