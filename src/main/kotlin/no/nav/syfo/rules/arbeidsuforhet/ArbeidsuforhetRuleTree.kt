...
internal fun RuleNode<ArbeidsuforhetRules, RuleResult>.yes(status: Status, ruleHit: ArbeidsuforhetRuleHit? = null) {
    yes(RuleResult(status, ruleHit?.ruleHit))
}

internal fun RuleNode<ArbeidsuforhetRules, RuleResult>.no(status: Status, ruleHit: ArbeidsuforhetRuleHit? = null) {
...
fun getRule(rules: ArbeidsuforhetRules): Rule<ArbeidsuforhetRules> {
    return when (rules) {
        ...
        ArbeidsuforhetRules.HOVEDDIAGNOSE_ELLER_FRAVAERSGRUNN_MANGLER -> houvedDiagnoseEllerFraversgrunnMangler
