...
fun main() {
    ...
    arbeidsuforhetRuleTree.traverseTree(builder, "root", "root")
...
private fun TreeNode<ArbeidsuforhetRules, RuleResult>.traverseTree(
    ...
    when (this) {
        ...
        is RuleNode -> {
            ...
            if (yes is ResultNode) {
                val childResult = (yes as ResultNode<ArbeidsuforhetRules, RuleResult>).result.status
                val childKey = "${currentNodeKey}_$childResult"
                builder.append("    $thisNodeKey($rule) -->|Yes| $childKey($childResult)${getStyle(childResult)}\n")
            } else {
                val childRule = (yes as RuleNode<ArbeidsuforhetRules, RuleResult>).rule
            ...
            if (no is ResultNode) {
                val childResult = (no as ResultNode<ArbeidsuforhetRules, RuleResult>).result.status
                val childKey = "${currentNodeKey}_$childResult"
                builder.append("    $thisNodeKey($rule) -->|No| $childKey($childResult)${getStyle(childResult)}\n")
            } else {
                val childRule = (no as RuleNode<ArbeidsuforhetRules, RuleResult>).rule
                val childKey = "${currentNodeKey}_$childRule"
