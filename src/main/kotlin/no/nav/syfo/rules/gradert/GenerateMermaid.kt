...
fun main() {
    ...
    gradertRuleTree.traverseTree(builder, "root", "root")
...
private fun TreeNode<GradertRules, RuleResult>.traverseTree(
    ...
    when (this) {
        ...
        is RuleNode -> {
            ...
            if (yes is ResultNode) {
                val childResult = (yes as ResultNode<GradertRules, RuleResult>).result.status
                val childKey = "${currentNodeKey}_$childResult"
                builder.append("    $thisNodeKey($rule) -->|Yes| $childKey($childResult)${getStyle(childResult)}\n")
            } else {
                val childRule = (yes as RuleNode<GradertRules, RuleResult>).rule
            ...
            if (no is ResultNode) {
                val childResult = (no as ResultNode<GradertRules, RuleResult>).result.status
                val childKey = "${currentNodeKey}_$childResult"
                builder.append("    $thisNodeKey($rule) -->|No| $childKey($childResult)${getStyle(childResult)}\n")
            } else {
                val childRule = (no as RuleNode<GradertRules, RuleResult>).rule
                val childKey = "${currentNodeKey}_$childRule"
